...
function AdminsTab() {
  ...
  const promoteMut = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Amministratore aggiunto' });
    },
  ...
  const demoteMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['/web-api/admin/admins'] }); toast({ title: 'Amministratore rimosso' }); },
    onError: (e: any) => toast({ title: 'Errore', description: e.message, variant: 'destructive' }),
  });
  ...
  return (
    <div className="space-y-4 mt-3">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => setDialog(true)}><Plus className="h-4 w-4 mr-1" />Aggiungi admin</Button>
      </div>

      {isLoading && <div className="text-center py-6 text-muted-foreground">Caricamento...</div>}
      ...
      <div className="space-y-2">
          ...
          <Card key={a.id}>
            <CardContent className="p-3 flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <Button size="icon" variant="ghost" className="text-destructive" onClick={() => { if (confirm('Rimuovere accesso admin?')) demoteMut.mutate(a.id); }}>
        ...
        {admins.length === 0 && !isLoading && <p className="text-center py-6 text-muted-foreground">Nessun amministratore</p>}
      </div>

      <Dialog open={dialog} onOpenChange={setDialog}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>Aggiungi amministratore</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              <Label>Email (utente registrato)</Label>
              <Input type="email" placeholder="nome@esempio.com" value={email} onChange={e => setEmail(e.target.value)} />
            </div>
            <div className="flex items-center gap-3">
              <Switch checked={isMaster} onCheckedChange={setIsMaster} />
              <Label>Master Admin (può eliminare ordini e gestire admin)</Label>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={() => promoteMut.mutate()} disabled={!email || promoteMut.isPending}>
              {promoteMut.isPending ? 'Salvataggio...' : 'Aggiungi'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function PickupAddressesTab() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { inv(); setDialog({ open: false, editing: null }); toast({ title: 'Indirizzo salvato' }); },
    onError: (e: any) => toast({ title: 'Errore', description: e.message, variant: 'destructive' }),
  });

  const deleteMut = useMutation({
    mutationFn: (id: string) => adminApi.deletePickupAddress(id),
    onSuccess: () => { inv(); toast({ title: 'Indirizzo eliminato' }); },
    onError: (e: any) => toast({ title: 'Errore', description: e.message, variant: 'destructive' }),
  });
  ...
  return (
    <div className="space-y-4 mt-3">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => openDialog()}><Plus className="h-4 w-4 mr-1" />Nuovo indirizzo</Button>
      </div>
              ...
              <div className="flex items-start justify-between gap-2 flex-wrap">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {a.isDefault && <Badge variant="default" className="text-xs">Principale</Badge>}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1 ml-6">{a.fullAddress}</p>
                  {a.contactName && <p className="text-xs text-muted-foreground ml-6">Contatto: {a.contactName} {a.contactPhone}</p>}
                </div>
                <div className="flex gap-1 shrink-0">
                  <Button size="icon" variant="ghost" onClick={() => openDialog(a)}><Pencil className="h-3.5 w-3.5" /></Button>
                  <Button size="icon" variant="ghost" className="text-destructive" onClick={() => { if (confirm('Eliminare?')) deleteMut.mutate(a.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </div>
            </CardContent>
          </Card>
        ))}
        {addresses.length === 0 && !isLoading && <p className="text-center py-6 text-muted-foreground">Nessun indirizzo</p>}
      </div>

      <Dialog open={dialog.open} onOpenChange={v => !v && setDialog({ open: false, editing: null })}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{dialog.editing ? 'Modifica indirizzo' : 'Nuovo indirizzo'}</DialogTitle></DialogHeader>
          <div className="space-y-3 py-2">
            <div className="space-y-1.5"><Label>Etichetta</Label><Input value={form.label} onChange={e => set('label', e.target.value)} placeholder="es. Magazzino principale" /></div>
            <div className="space-y-1.5"><Label>Indirizzo completo</Label><Input value={form.fullAddress} onChange={e => set('fullAddress', e.target.value)} /></div>
            <div className="space-y-1.5"><Label>Città</Label><Input value={form.city} onChange={e => set('city', e.target.value)} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label>Nome contatto</Label><Input value={form.contactName} onChange={e => set('contactName', e.target.value)} /></div>
              <div className="space-y-1.5"><Label>Telefono contatto</Label><Input value={form.contactPhone} onChange={e => set('contactPhone', e.target.value)} /></div>
            </div>
            <div className="flex items-center gap-3">
              <Switch checked={form.isDefault} onCheckedChange={v => set('isDefault', v)} />
              <Label>Indirizzo principale</Label>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDialog({ open: false, editing: null })}>Annulla</Button>
            <Button onClick={() => saveMut.mutate(form)} disabled={saveMut.isPending}>{saveMut.isPending ? 'Salvataggio...' : 'Salva'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function AssociationsTab() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { inv(); setSourceId(''); setTargetId(''); toast({ title: 'Associazione creata' }); },
    onError: (e: any) => toast({ title: 'Errore', description: e.message, variant: 'destructive' }),
  });
    ...
    <div className="space-y-4 mt-3">
      <Card>
        <CardContent className="p-4 space-y-3">
          <p className="text-sm font-medium">Nuova associazione</p>
          <div className="grid grid-cols-2 gap-2">
            <Select value={sourceId} onValueChange={setSourceId}>
              <SelectTrigger><SelectValue placeholder="Prodotto sorgente" /></SelectTrigger>
              <SelectContent>{products.map((p: any) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}</SelectContent>
            </Select>
            <Select value={targetId} onValueChange={setTargetId}>
              <SelectTrigger><SelectValue placeholder="Prodotto correlato" /></SelectTrigger>
          ...
          <Button size="sm" onClick={() => createMut.mutate()} disabled={!sourceId || !targetId || createMut.isPending} className="w-full">
            <Link2 className="h-4 w-4 mr-1" />Crea associazione
          </Button>
        </CardContent>
      </Card>

      {loadAssocs && <div className="text-center py-6 text-muted-foreground">Caricamento...</div>}
      ...
      <div className="space-y-2">
        ...
        {assocs.length === 0 && !loadAssocs && <p className="text-center py-6 text-muted-foreground">Nessuna associazione</p>}
      </div>
    </div>
  );
}
    ...
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-4">
      ...
      <Tabs defaultValue="admins">
        <TabsList>
          ...
          <TabsTrigger value="addresses">Indirizzi ritiro</TabsTrigger>
          <TabsTrigger value="associations">Prodotti correlati</TabsTrigger>
        </TabsList>
