...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // NOTE: createOrder is NOT available with Yandex Taxi API (only Cargo API)
  // This endpoint is disabled until we have access to Cargo API
  /*
  // POST /api/admin/orders/:id/yandex-delivery - Crea ordine delivery Yandex (ADMIN ONLY - REQUIRES CARGO API)
  app.post("/api/admin/orders/:id/yandex-delivery", verifyTelegramInitData, requireAdmin, async (req, res) => {
    try {
      res.status(501).json({ 
        error: 'Not Implemented',
        message: 'Chiamata automatica taxi non disponibile. Usa il widget Yandex Taxi per chiamare manualmente.'
      });
    } catch (error) {
  ...
  */
  ...
  // Placeholder for future Cargo API implementation
  app.post("/api/admin/orders/:id/yandex-delivery", verifyTelegramInitData, requireAdmin, async (req, res) => {
    res.status(501).json({ 
      error: 'Not Implemented',
      message: 'Chiamata automatica taxi non disponibile con API Taxi. Richiede accesso a Yandex Cargo API.'
    });
  });
  
  // NOTE: getOrderStatus and cancelOrder not available with Taxi API (only Cargo API)
  
  // Placeholder - not available with Taxi API
  app.get("/api/admin/orders/:id/yandex-delivery-status", verifyTelegramInitData, requireAdmin, async (req, res) => {
    res.status(501).json({ 
      error: 'Not Implemented',
      message: 'Status tracking non disponibile con API Taxi. Richiede accesso a Yandex Cargo API.'
    });
  });
  ...
  app.post("/api/admin/orders/:id/yandex-delivery-cancel", verifyTelegramInitData, requireAdmin, async (req, res) => {
    res.status(501).json({ 
      error: 'Not Implemented',
      message: 'Annullamento automatico non disponibile con API Taxi. Richiede accesso a Yandex Cargo API.'
    });
  });
