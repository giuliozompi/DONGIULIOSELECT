...
5.  **Yandex Go API v2**: General taxi-based delivery service (https://b2b.taxi.yandex.net) with cargo delivery, flexible requirements, and real-time courier tracking. **Important Discovery**: Both Yandex Dostavka and Yandex Go use the **same API endpoint** (https://b2b.taxi.yandex.net/b2b/cargo/integration/v2/) and can **share the same Bearer token** (`YANDEX_DOSTAVKA_TOKEN`). The service now uses the Dostavka token for both integrations. **Response Parsing**: Yandex API returns nested price objects (`{total_price, total_price_with_vat, surge_ratio, currency}`) and time intervals (`pickup_interval`, `delivery_interval`) which are extracted and formatted by the backend before display. **Validation**: Backend validates pickup and delivery coordinates to ensure they are non-null and not [0,0] before calling API, returning user-friendly Italian error messages if validation fails.
