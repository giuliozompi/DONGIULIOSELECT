...
- **Gamification**: A fortune wheel system rewards users with spin tokens upon order completion. Fortune wheel bonus is applied first; abandoned cart code cascades on the already-discounted price (cumulative). Welcome code is NOT cumulative with the fortune wheel bonus.
- **Promo Code System**: Supports manual promo code generation for specific users with customizable discount percentages and expiry dates. Features friendly, emoji-enhanced error messages in Russian for different scenarios (already used, expired, not found, wrong owner) to enhance user experience.
...
- **Triple Delivery System**: Integrates with Yandex Dostavka (express courier), Yandex Go (business cargo), and CDEK (nationwide pickup point network) for real-time pricing, tracking, and automated order management. Yandex services share pick-up address management with DaData integration, support claim cancellation, info retrieval, and performer tracking. CDEK integration features OAuth 2.0 authentication with 1-hour token caching, PVZ (pickup point) selection with city search and real-time availability, tariff calculation based on cargo dimensions and weight, and webhook support for delivery status updates. Includes production-ready features like idempotency keys, correlation IDs, exponential backoff, pre-flight validation, structured logging, and smart fallback. A robust webhook and tracking infrastructure with dedicated database tables (`orderPoints`, `webhookEvents`, `courierTracking`, `cdekPvzCache`) and E.164 phone validation ensures reliable delivery management. **Payment flow**: Don Giulio always pays for delivery (no payment_on_delivery or fiscalization used), while customers pay for products via YooKassa. Yandex Go uses POST method for `/claims/info` endpoint. CDEK main tariff codes: 136 (warehouse-to-warehouse), 137 (warehouse-to-door), 483 (express).
...
- **Welcome Notification System**: Targets users who opened the app at least once but never placed an order and never added items to the cart. Sends a one-time welcome promo code (format: `BENVENUTO-XXXXXX`) via Telegram and Email with 10% discount on first purchase of minimum 5,000 ₽. Code is single-use, non-combinable with bonuses, valid 30 days. Runs as 24h cron (10:00-20:00 MSK). Tracked in `welcomeNotifications` DB table, visible in the Admin Logs panel under the "Коммерческие" tab.
- **Notification Logging**: Full audit trail for all notification events stored in DB tables (`orderNotificationLogs`, `abandonedCartNotifications`, `reengagementNotifications`, `welcomeNotifications`). Admin Logs panel has three tabs: "Действия адм." (admin actions), "По заказам" (order notifications), and "Коммерческие" (cart reminders, re-engagement, welcome promo codes).
...
6.  **CDEK API v2**: For nationwide pickup point (PVZ) delivery services, using OAuth 2.0 authentication with client credentials flow. Supports tariff calculation, order creation, status tracking, and webhook notifications.
7.  **Meta WhatsApp Business Platform (Cloud API)**: For order notifications and status updates via WhatsApp templates.
8.  **DaData.ru Address Autocomplete**: For Russian address validation and standardization.
9.  **Resend Email API**: For transactional email notifications to customers and managers.
10. **Neon Database**: Serverless PostgreSQL hosting.
11. **NPM Dependencies**: Libraries for UI (Radix UI, Tailwind CSS), forms (react-hook-form, Zod), and data persistence (Drizzle ORM).