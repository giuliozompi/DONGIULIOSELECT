...
5.  **Yandex Go Доставка для бизнеса**: Business taxi delivery service (https://b2b.taxi.yandex.net/b2b/cargo/integration/v2/) with flexible requirements and real-time tracking. **IMPORTANT**: Yandex Go and Yandex Dostavka are **SEPARATE** services requiring **SEPARATE OAuth tokens**. **Authentication**: Supports dual-mode authentication - OAuth Client Credentials flow (YANDEX_GO_CLIENT_ID + YANDEX_GO_CLIENT_SECRET) or Static API Token (YANDEX_GO_TOKEN). **Token source**: Яндекс Go Доставка для бизнеса cabinet → Интеграция → Create OAuth token with permissions "cargo:write" and "cargo:read". **Secrets**: `YANDEX_GO_TOKEN` (OAuth Bearer token), optionally `YANDEX_GO_CLIENT_ID` and `YANDEX_GO_CLIENT_SECRET` for OAuth flow. **API Endpoints**: Same as Yandex Dostavka - `/offers/calculate` for pricing, `/claims/create` for order creation, `/claims/info` for status. **Error Handling**: 403 errors indicate token lacks proper permissions - must have "cargo:write" and "cargo:read" scopes. Clear Italian error messages guide users to correct configuration. **OAuth Service**: Automatic token refresh, retry logic with exponential backoff, graceful fallback to static token. **Validation**: Backend validates pickup and delivery coordinates to ensure they are non-null and not [0,0] before calling API, returning user-friendly Italian error messages if validation fails.
