...
5.  **Yandex Go Cargo API v2**: Taxi-based delivery service (https://b2b.taxi.yandex.net/b2b/cargo/integration/v2/) with cargo delivery, flexible requirements, and real-time courier tracking. **Important Discovery**: Yandex Go CARGO and Yandex Dostavka use the **SAME API endpoint** (https://b2b.taxi.yandex.net) but require **separate authentication tokens** with different service permissions: `YANDEX_GO_TOKEN` for taxi-based cargo delivery vs `YANDEX_DOSTAVKA_TOKEN` for express courier service. The API differentiates services based on token permissions and request parameters. **Token source**: Yandex Delivery Corporate Dashboard → Интеграции → Получить токен. **Note**: `http://b2b-api.go.yandex.ru` is a different API for B2B user management (departments, limits), NOT for cargo delivery. **Response Parsing**: Yandex API returns nested price objects (`{total_price, total_price_with_vat, surge_ratio, currency}`) and time intervals (`pickup_interval`, `delivery_interval`) which are extracted and formatted by the backend before display. **Validation**: Backend validates pickup and delivery coordinates to ensure they are non-null and not [0,0] before calling API, returning user-friendly Italian error messages if validation fails.
