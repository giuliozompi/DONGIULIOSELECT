...
- **Category & Product Visibility Control (November 2025)**: Master Admin can hide categories and products from the public customer-facing interface while keeping them visible in the admin panel. Features include:
  - Eye/EyeOff toggle buttons for one-click visibility control
  - Visual indicators for hidden items: opacity effect and "Nascosto" badge (admin-only)
  - Secure API design: Hidden items are completely inaccessible to non-admin users, even with URL parameter manipulation
  - Full audit trail: All visibility changes logged with oldVisibility and newVisibility fields
  - Database schema: `isVisible` boolean field (default true) on both categories and products tables
  - Admin-gated includeHidden parameter for viewing hidden items in admin panel only
